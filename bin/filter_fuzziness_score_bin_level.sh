...
### 1.2 linker bed ###
  ...
  for sample in ${SAMPLE};do
      ...
      join -1 1 -2 4 $output_dir/${sample}.linker_id.txt  $output_dir/${sample}.linker.bed | awk -v OFS="\t" '{print $2,$3,$4}' > $output_dir/${sample}.linker.filter.bed 
...
### 2.1 bin ###
    ...
    for sample in ${SAMPLE};do
    echo "bin ${sample}"
        bedtools makewindows -b $output_dir/${sample}.nucleosome.filter.bed  -n 10 -i winnum > \
        $output_dir/${sample}.nucleosome.binned.bed  2> $logDir/${sample}.nucleosome.bin.log &
        bedtools makewindows -b $output_dir/${sample}.linker.filter.bed  -n 10 -i winnum > \
    ...
    time ( for sample in ${SAMPLE};do
        for j in nucleosome linker;do
            for i in `seq 1 10`;do
                ...
                # modify here
                bedtools intersect -wo -a danpos/normed/${sample}/pooled/out.uniq.rmdup.nameSorted.smooth.wig.bed -b $output_dir/${sample}.$j.bin_$i.bed \
        ...
        for mA_list in ${MA_LIST};do
            for j in nucleosome linker;do
                for i in `seq 1 10`;do
                    ...
                    OccuSum=`awk '{sum += $5*$NF} END {print sum}' $output_dir/${sample}.$j.intersect.occu.bin_$i.bed`
