...
################################################
#File Name: run.optimize.sh
#Author: LiTing
#Mail: liting@stu.pku.edu.cn
#Created Time: Thu 24 Feb 2022 04:34:40 PM CST
################################################
...
### 2022-02-24 feature analysis ###
    ...
    [ -d /home/user/data/lit/project/6mA/feature/iNPS/method_1 ] || mkdir -p /home/user/data/lit/project/6mA/feature/iNPS/method_1
    [ -d log/iNPS/method_1 ] || mkdir -p log/iNPS/method_1
    output_dir=/home/user/data/lit/project/6mA/feature/iNPS/method_1
    log_dir=log/iNPS/method_1
    ...
    for sample in $(for i in `seq 1 2`;do echo PC10_LY_MCC-M$i;done);do
        for j in nucleosome linker; do
            echo "processing ${sample}.$j.bed "
            bedtools intersect -wb -a $output_dir/${sample}.${j}.bed -b sig_gt_2_fc0_cov10_for_lt_wt1.bed | \
            awk -v OFS="\t" '{print $4,$5,$6,$7}' > $output_dir/${sample}.${j}.intersect.bed &

            bedtools getfasta -fi $ce11_dir/ce11.fa -bed $output_dir/${sample}.${j}.bed > \
            $output_dir/${sample}.${j}.fasta &
        done 
        wait
    done )
    ...
    for sample in $(for i in `seq 1 2`;do echo PC10_LY_MCC-M$i;done);do
        for j in nucleosome linker; do
            FreqSum=`less $output_dir/${sample}.${j}.intersect.bed | awk '{sum += $4} END {print sum}'`
            mA=`less $output_dir/${sample}.${j}.intersect.bed | wc -l`
            A=`grep -i -o a $output_dir/${sample}.${j}.fasta | wc -l`
            echo -e "${sample}.${j}\t${mA}\t${A}\t${FreqSum}" >> $output_dir/level.txt
        done 
    done 
...
# nohup bash run.optimize.sh > log/iNPS/method_1/run.log 2>&1 &
